...
export const createCourse = asyncHandler(async (req, res, next) => {
  ...
  if (!course) {
    return next(
      new AppError('complain could not be created, please try again', 400)
    );
  }
  ...
  if (req.file) {
    try {
      const result = await cloudinary.v2.uploader.upload(req.file.path, {
        folder: 'GS', // Save files in a folder named lms
      });
...
export const getLecturesByCourseId = asyncHandler(async (req, res, next) => {
  ...
  if (!course) {
    return next(new AppError('Invalid complain id or complain not found.', 404));
  }

  res.status(200).json({
    success: true,
    message: 'complain lectures fetched successfully',
...
export const addLectureToCourseById = asyncHandler(async (req, res, next) => {
  ...
  if (!course) {
    return next(new AppError('Invalid complain id or complain not found.', 400));
  }
  ...
  res.status(200).json({
    success: true,
    message: 'complain lecture added successfully',
    course,
  });
});
...
export const removeLectureFromCourse = asyncHandler(async (req, res, next) => {
  ...
  if (!lectureId) {
    return next(new AppError('Avidance ID is required', 400));
  }
  ...
  if (!course) {
    return next(new AppError('Invalid ID or complain does not exist.', 404));
  }
  ...
  if (lectureIndex === -1) {
    return next(new AppError('Avidance does not exist.', 404));
  }
  ...
  res.status(200).json({
    success: true,
    message: 'complain Avidance removed successfully',
  });
});
...
export const deleteCourseById = asyncHandler(async (req, res, next) => {
  ...
  if (!course) {
    return next(new AppError('complain with given id does not exist.', 404));
  }
